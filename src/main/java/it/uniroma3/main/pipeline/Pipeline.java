...
public class Pipeline {
	...
	if (parsedDump != null){
	    try {
		File outputParsed = new File(parsedDump);
		outputParsed.getParentFile().mkdirs();
		this.parsedDumpWriter = new PrintStream(new FileOutputStream(outputParsed.getAbsolutePath()), false, "UTF-8");
	...
	if (augmentedDump != null){
		...
		this.augmentedDumpWriter = new PrintStream(new FileOutputStream(outputAugmented.getAbsolutePath()), false, "UTF-8");
	    } catch (UnsupportedEncodingException | FileNotFoundException e) {
		e.printStackTrace();
	    }
	}
	...
	while (!(lines = dumpFileReader.nextChunk(chunckSize)).isEmpty() && cont < totArticle) {
	    ...
	    List<WikiArticle> chunk = lines.parallelStream()
	    ...
	    /*
	    long start_pars_wrt_time = System.currentTimeMillis();
	    chunk.parallelStream()
	    .forEach(s -> parsedDumpWriter.println(s.toJson()));
	    long end_pars_wrt_time = System.currentTimeMillis();
	    long pars_wrt_time = TimeUnit.MILLISECONDS.toSeconds(end_pars_wrt_time - start_pars_wrt_time);
	    System.out.print("\tStored in: " + pars_wrt_time + " sec.\t");
	     */
