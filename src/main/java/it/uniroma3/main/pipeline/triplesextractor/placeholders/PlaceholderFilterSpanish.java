...
public class PlaceholderFilterSpanish extends PlaceholderFilter {
  ...
  public List<Pattern> fillPositions() {
    return Arrays.asList(
        Pattern.compile("\\b(" + "sur(-)?oeste|" + "sur(-)?este|" + "norte(-)?este|"
            + "norte(-)?oeste|" + "sur(-)?central|" + "norte(-)?central|" + "oeste(-)?central|"
            + "sur(-)?central|" + "central(-)?norte|" + "central(-)?sur|" + "central(-)?este|"
            + "central(-)?oeste" + ")\\b", Pattern.CASE_INSENSITIVE),
        Pattern.compile("\\b(" + "norte|" + "sur|" + "oeste|" + "este)\\b",
            Pattern.CASE_INSENSITIVE));
  }

  @Override
  public List<Pattern> fillLengths() {
    return Arrays.asList(Pattern.compile("#YEAR#\\s?(km|kilómetro)\\b"),
        Pattern.compile("\\d+(\\s\\d+)*\\s?(km|kilómetro)\\b"));
  }

  @Override
  public List<Pattern> fillMonths() {
    return Arrays.asList(Pattern.compile("\\b(enero|febrero|marzo|abril|mayo|junio|julio|agosto|"
        + "septiembre|octubre|noviembre|diciembre)\\b", Pattern.CASE_INSENSITIVE));
  }

  @Override
  public List<Pattern> fillOrdinals() {
    return Arrays.asList(Pattern.compile("\\b\\d*1st\\b"), Pattern.compile("\\b\\d*2nd\\b"),
        Pattern.compile("\\b\\d*3rd\\b"), Pattern.compile("\\b(\\d)*\\dth\\b"),
  ...
  public List<Pattern> fillDates() {
    return Arrays.asList(Pattern.compile("#YEAR#(\\s|,\\s)#DAY#"),
        Pattern.compile("#DAY#(\\s|,\\s)#YEAR#"), Pattern.compile("\\b#MONTH# de #YEAR#\\b"),
        Pattern.compile("\\b#DAY# de #MONTH#\\b"));
  }

  @Override
  public Set<String> fillConjuctions() {
    return new HashSet<String>(Arrays.asList("y", ",", "a"));
  }
