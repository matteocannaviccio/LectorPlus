...
import it.uniroma3.main.util.Token;
...
public class FSMSeed {
  ...
  private static final List<String> FINAL_LIST = Arrays.asList("VBG", "JJ", "CC", ",", "VB", "VBN",
      "VBD", "VBZ", "WP", "WDT", "WRB", "TO", "IN", ".");
  ...
  /**
   * Method to implement a finite state machine to capture the seeds from the first sentence. S1 ->
   * RR state S2 -> numerical state S3 -> noun state S4 -> adjective state S5 -> conjunction state
   * S6 -> pos state S7 -> proper noun state S8 -> in state S9 -> plural nouns
   * 
   * @return
   */
  ...
  /**
   * Remove the "subject" part of the sentence. It considers the occurrence of one of "is-a" verb in
   * the text.
   * 
