...
/**
 * This parser is able to assign a specific type to the article using its wikid or original
 * wiki-markup.
 * 
 * @author matteo
 *
...
public class ArticleTyper {
  ...
  /**
   * Check if the article can be a REDIRECT using the whole text. if it contains the reference
   * #REDIRECT.
   * 
   * @param blocks
   * @return
  ...
  /**
   * Check if the article can be a DISAMBIGUAITON using the text in the abstract section or a
   * template in the whole article.
   * 
   * @param blocks
   * @param article
   * @return
   ...
   * 
   * Generally particular types of articles have particular wikids such as "Category:Italian
   * footballer" or "Discussion:something else".
   * 
   * @param article
   * @return
   */
  public ArticleType findArticleType(WikiArticle article) {
    ...
    if (article.getWikid().startsWith("Draft:")
        || article.getWikid().startsWith("Parroquia_Junquillal")) {
