...
public class TypesResolver {
  ...
  public TypesResolver(Lang language) {
    ...
    /*
     * we use different dictionary of types based on the language. ie. for English we have three
     * more dictionaries. the "ref" index are always the english ones.
     */
    switch (language) {
      case en:
        indexOriginal = getIndexOrCreate(Configuration.getTypesIndex(),
            Configuration.getSourceMainInstanceTypes());
        indexAirpedia = getIndexOrCreate(Configuration.getAirpediaIndex(),
            Configuration.getSourceAirpediaTypes());
        indexSDTyped = getIndexOrCreate(Configuration.getSDTypesIndex(),
            Configuration.getSourceSDTypedInstanceTypes());
        indexLHD = getIndexOrCreate(Configuration.getLHDTypesIndex(),
            Configuration.getSourceLHDInstanceTypes());
        indexDBTax = getIndexOrCreate(Configuration.getDBTaxTypesIndex(),
            Configuration.getSourceDBTaxInstanceTypes());
  ...
  /**
   * Here we express the rules that are used to assign a type to an input entity. We use the
   * standard instance types dictionary. In case it does not contains a type, for english and
   * germany we can use two further dictionaries.
   * 
   * @param wikid
   * @return
   */
  ...
  public static void main(String[] args) {
    ...
    try {
      BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(
          new FileOutputStream("/Users/matteo/Desktop/dbpedia_ontology.tsv"), "UTF-8"));
      indexOriginal.matchAllPairs().stream().forEach(s -> {
        try {
    ...
    /*
     * String entity = "Thomas_Hyde"; System.out.println("USED --> " + t.assignTypes(entity));
     * 
     * System.out.println("\nTypes in orginal mapping: "); t.getTypes(entity,
     * t.getIndexOriginal()).forEach(System.out::println);
     * 
     * System.out.println("\nTypes in Airpedia: "); t.getTypes(entity,
     * t.getIndexAirpedia()).forEach(System.out::println);
     */
  }
