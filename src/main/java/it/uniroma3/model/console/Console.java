...
public class Console{
    ...
    // commands
    enum Command {load, quit, help, relprior, relrank, dbp, relex, model, tRc, pRc, tpRc, pTRc};
    ...
    private static String usage = 
	    ...
	    String.format("%-20s %-65s %s\n", "[phrase -> types, relations]", "/ptlf/<phrase>", "return the typed relations that label the phrase").toString() +
	...
	ModelIndexer model_indexes = new ModelIndexer(Configuration.getLectorFolder() + "/model_indexer_"+PERCENTAGE+"/", PERCENTAGE);
	...
	while (running){
		...
		case relrank: //RELATION RANKING
		    CounterMap<String> rels = model_indexes.matchAllSimple(IndexType.relations2typedphrases);
		...
		case relex: //RELATION EXAMPLES
		    ...
		    System.out.printf("\t%-5s %-5s %-"+spanRel+"s %-"+max+"s %-15s %-15s %-15s %-15s %-15s %s\n", "", "", "RELATION", "TYPED-PHRASE", "", "freq.", "%rel", "%LAB", "%UNL", "c(tp, TOT)");
		    System.out.println("--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		    ...
		    int count = 1;
		    for (Map.Entry<String, Integer> typh : ranked_typed_phrases.entrySet()){
			CounterMap<String> typ_rels = model_indexes.getKeyValuesCounts(typh.getKey(), IndexType.typedphrases2relations);

			// prendi la typed phrase solo se identifica davvero la relazione in input (evita falsi positivi di tipo 2)
			int tot = typ_rels.calculateSum();
			boolean firstNone = false;
			if (Ranking.getRanking(typ_rels).entrySet().iterator().next().getKey().equals(input)){
			    firstNone = true;
			}
			...
			if (Ranking.getRanking(typ_rels).entrySet().iterator().next().getKey().equals(input)){
			    ...
			    if (firstNone)
				System.out.printf("\t%-5s %-5s %-"+spanRel+"s %-"+max+"s %-15s %-15s %-15s %-15s %-15s %s\n", count+")", "*", input, typh.getKey().replaceAll("\t", " "), "--->", freq, String.format( "%.2f", rel), String.format( "%.2f", lab), String.format( "%.2f",unl), tot);
			    else
				System.out.printf("\t%-5s %-5s %-"+spanRel+"s %-"+max+"s %-15s %-15s %-15s %-15s %-15s %s\n", count+")", "", input, typh.getKey().replaceAll("\t", " "), "--->", freq, String.format( "%.2f", rel), String.format( "%.2f", lab), String.format( "%.2f",unl), tot);
			    count +=1;
			}
		...
		case pRc: //PHRASE -> RELATION COUNT
			...
			for (Map.Entry<String, Integer> rel : Ranking.getTopKRanking(res, 50).entrySet()){
			    System.out.printf("\t%-10s %-20s %-25s %-20s %s\n", rel.getValue(), String.format( "%.2f", (double)rel.getValue()/total ), rel.getKey(), "---->", phrase);
			}
		    }
		    break;
		...
		case pTRc: //PHRASE -> TYPES AND RELATIONS COUNT
		    ...
		    System.out.println("List of relations and pairs of types associated with the phrase: " + phrase);
		    if ((res == null || res.isEmpty()))
			System.out.println("Found: 0 instances.");
		    else{
			int total = res.calculateSum();
			...
			for (Map.Entry<String, Integer> rel : Ranking.getTopKRanking(res, 50).entrySet()){
			    ...
			    System.out.printf("\t%-10s %-20s %-25s %-20s %-20s %-15s %s\n", rel.getValue(), String.format( "%.2f", (double)rel.getValue()/total ), relation, "---->", subjectType, phrase, objectType);
			}
		    }
		    break;
		...
		case tpRc: //TYPED PHRASE -> RELATION COUNT  
			...
			objType = "[" + tokens[tokens.length -1].substring(0, 1).toUpperCase() + tokens[tokens.length-1].substring(1) + "]";
		    ...
		    System.out.println("List of relations associated with the typed phrase: " + subType + " " + phrase + " " + objType);
		    if ((res == null || res.isEmpty()))
			System.out.println("Found: 0 instances.");
		    else{
			int total = res.calculateSum();
			...
			for (Map.Entry<String, Integer> rel : Ranking.getTopKRanking(res, 50).entrySet()){
			    ...
			    double variance = 2*Math.sqrt(((double)(p*(1-p))/samples) * (double)(total-samples)/(total-1)) ;
			    if (!rel.getKey().equals("NONE")){
				totLAB += rel.getValue();
				System.out.printf("\t%-10s %-10s %-20s %-25s %-20s %s\n", rel.getValue(), String.format( "%.2f", (double)rel.getValue()/total ), String.format( "%.2f", p) + " +- " + String.format( "%.3f", variance ), rel.getKey(),"---->", subType + " " + phrase + " " + objType);
			    }else{
				totNONE += rel.getValue();
				System.out.printf("\t%-10s %-10s %-20s %-25s %-20s %s\n", rel.getValue(), String.format( "%.2f", (double)rel.getValue()/total ), "-", rel.getKey(),"---->", subType + " " + phrase + " " + objType);
			    }
			}
		    }
		...
		case load:
		    ...
		    //modelTextExt = Model.getNewModel(new DBModel(Configuration.getDBModel()), "model_triples", 1, 100, ModelType.ModelTextExt, true);
		    
		    System.out.println("\t-> Init OnlyPositive Model ...");
		    modelOnlyPositive = Model.getNewModel(new DBModel(Configuration.getDBModel()), "model_triples", 1, 0, ModelType.NaiveBayes, 0.4);
		    
		    System.out.println("\t-> Init WithNone Model ...");
		    modelWithNone = Model.getNewModel(new DBModel(Configuration.getDBModel()), "model_triples", 1, 100, ModelType.NaiveBayes, 0.4);	
		    
		    System.out.println("\t-> Init Middle Model ...");
		    modelMiddle = Model.getNewModel(new DBModel(Configuration.getDBModel()), "model_triples", 1, 25, ModelType.NaiveBayes, 0.4);
		    break;
		...
		case tRc: //TYPED PHRASE -> RELATION COUNT
		    ...
		    System.out.println("List of relations associated with the typed phrase: " + subType + " " + objType);
		    if ((res == null || res.isEmpty()))
			System.out.println("Found: 0 instances.");
		    else{
			int total = res.calculateSum();
			System.out.println("Found: "+ res.size() + " with a total of " + total);
			for (Map.Entry<String, Integer> rel : Ranking.getTopKRanking(res, 50).entrySet()){
			    System.out.printf("\t%-10s %-20s %-25s %-20s %s\n", rel.getValue(), String.format( "%.2f", (double)rel.getValue()/total ), rel.getKey(),"---->", subType + " " + objType);
			}
		    }
		    break;
