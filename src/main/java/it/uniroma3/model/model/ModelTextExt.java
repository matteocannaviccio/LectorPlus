...
public class ModelTextExt extends Model {
     ...
     * 
     */
    protected Map<String, CounterMap<String>> phrases2relations = new HashMap<String, CounterMap<String>>();
	...
	try {
		...
		for (Map.Entry<String, Double[]> phrase : Ranking.getDoubleKRanking(relation.getValue(), 9, -1).entrySet()){
		    ...
		    values[2] = String.valueOf(phrase.getValue()[0]); 	// c(tpR)
		    values[3] = String.valueOf(phrase.getValue()[1]); 	// c(tpL)
		    values[4] = String.valueOf(phrase.getValue()[2]); 	// c(tpU)
		    values[5] = String.valueOf(phrase.getValue()[3]); 	// c(pR)
		    values[6] = String.valueOf(phrase.getValue()[4]); 	// c(pL)
		    values[7] = String.valueOf(phrase.getValue()[5]); 	// c(pU)
		    values[8] = String.valueOf(phrase.getValue()[6]); 	// P(tpR)
		    values[9] = String.valueOf(phrase.getValue()[7]); 	// P(tpRU)
		    values[10] = String.valueOf(phrase.getValue()[8]); 	// P(pR)
		    values[11] = String.valueOf(phrase.getValue()[9]); 	// P(pRU)
		    values[12] = String.valueOf(phrase.getValue()[10]); // NONE
    ...
    @Override
    public Pair<String, Double> predict(String subject_type, String phrase_placeholder, String object_type) {
