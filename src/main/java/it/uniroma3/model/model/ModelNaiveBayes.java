...
public class ModelNaiveBayes extends Model {
     ...
     */
    public ModelNaiveBayes(DBLector db_model, String evidence_table, int minF, int percentage, ModelType modelType, double majorityThreshold) {
	...
	if (!prediction.equals("NONE") && probability < majorityThreshold){
	    ...
	    //System.out.printf("%s\t%s\t%s\t%s\t%s\n", typedPhrase, finalPrediction.key, probability, NegativePredictions.MAJRULE.name(), ranking.toString());
    ...
    /**
     * Returns the probability that the input typed-phrase describes the given relation
     * using NaiveBayes posterior calculation.
     * 
     * @param typedPhrase
     * @param relation
     * @return
	...
	if(this.typedPhrases2relations.containsKey(typedPhrase) && this.typedPhrases2relations.get(typedPhrase).containsKey(relation)){
